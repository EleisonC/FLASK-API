...
class CategorylistTestCase(unittest.TestCase):
    ...
    def test_category_creation(self):
        ...
        res = self.client.post('/categorylist/', data=self.categorylist)
    ...
    def test_api_can_get_all_categories(self):
        ...
        res = self.client.post('/categorylist/', data=self.categorylist)
        self.assertEqual(res.status_code, 201)
        res = self.client.get('/categorylist/')
        self.assertEqual(res.status_code, 200)
        self.assertIn('Lunch', str(res.data))

    def test_api_can_get_category_by_id(self):
        """Test API can get a single category by using """
        rv = self.client.post('/categorylist/', data=self.categorylist)
        self.assertEqual(rv.status_code, 201)
        result_in_json = json.loads(rv.data.decode('utf-8').replace("'","\""))
        result = self.client.get(
            '/categorylist/{}'.format(result_in_json['id'])
        )
    ...
    def test_category_can_be_edited(self):
        ...
        results = self.client.get('/categorylist/1')
    ...
    def test_categorylist_deletion(self):
        ...
        self.assertEqual(rv.status_code, 201)
        res = self.client.delete('/categorylist/1')
        self.assertEqual(res.status_code, 200)
        result = self.client.get('/categorylist/1')
